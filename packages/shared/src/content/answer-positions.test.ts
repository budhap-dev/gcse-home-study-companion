...
interface Topic { id: string; subjectId: string; questions: Question[] }
 ...
 */
const numeric = (q: Question) => (q.options ?? []).every((o) => /^\s*[-+]?\d[\d.,]*\s*[^\d]{0,12}$/.test(o.trim()))
