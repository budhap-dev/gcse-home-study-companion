...
**Status: rolled out to all nine subjects.** Requested 18 September 2026 and proved first on Physics *Moments, levers and gears*. As of 24 September 2026 every topic in every subject carries the block; Further Maths was last, its 19 new rows carrying it from the start and its three earlier calculus topics filled in when the subject was completed (PR #266). Music, expected to be thin, was not: every one of its 19 topics carries examples, because *why does this form exist* and *what is the exam actually paying for* turned out to be the questions a set work most needs answered. French, expected to be thinner still, was not either: its 50 topics carry 150 examples, because a language topic is met in a situation (the form at the campsite, the sign on the museum door, the group chat on Friday) and pays in the exam for one piece of grammar, and both can be pictured in a short table or, for a tense, a `verb-table` with the stem drawn apart from the endings. The rest of this document is the design, corrected where building it showed the design was wrong.
...
- **A test reports coverage per subject** — how many topics have `matters`, how many have examples, how many of those examples have a picture — so gaps are visible rather than silent. The same argument as showing unwritten rows as *Coming soon* instead of hiding them. **Retired 25 September 2026:** the Subjects page column that carried this count came off once every topic in every subject had the section; the test that failed on that day now asserts that no topic lacks it, so a new topic cannot arrive without one.
...
0. ~~**Build `size-compare` first**, and prove the design on one topic end to end.~~ **Done.** `size-compare` exists, the `why` field is on `Topic`, the card renders on the topic page, and Physics *Moments, levers and gears* is written. Read that one before writing another. As of 24 September 2026 every subject carries the section on every topic.
1. **New topics carry it from the start.** The next Further Maths rows should include it, so the standard is set while the volume is small. **Done:** every Further Maths row written on 24 September 2026 carried it.
