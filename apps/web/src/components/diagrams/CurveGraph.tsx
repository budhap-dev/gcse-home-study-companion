...
export function CurveGraph({ props, alt }: { props: Record<string, unknown>; alt: string }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ maxWidth: 440 }} role="img" aria-label={alt}>
      ...
      {numbers && [0, 0.25, 0.5, 0.75, 1].map((k) => <text key={`n${k}`} x={sx(k * xMax)} y={H - pad + 14} textAnchor="middle" fontFamily={FONT} fontSize="10" fill={INK_2}>{Number((k * xMax).toFixed(2))}</text>)}
      {markers.map((m, i) => (
        <g key={`m${i}`}>
          <line x1={sx(m.x)} y1={pad} x2={sx(m.x)} y2={H - pad} stroke={INK_2} strokeDasharray="4 4" />
          <text x={sx(m.x) + 4} y={pad + 12} fontFamily={FONT} fontSize="10" fill={INK_2}>{m.label}</text>
        </g>
      ))}
      {curves.map((c, i) => {
